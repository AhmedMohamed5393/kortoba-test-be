...
import { GetMoviesInputValidator } from "../validators/getMoviesInputValidator";
const tag = "movies-dashboard-be:movie:getMoviesMiddleware";
export class GetMoviesMiddleware implements NestMiddleware {
    public use(req: any, res: any, next: () => void) {
        try {
            const validateRequestBody = new GetMoviesInputValidator().validateInputs(req.query);
